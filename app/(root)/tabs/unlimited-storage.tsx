...
const UpgradeStorage = () => {
  ...
  useEffect(() => {
    const fetchUser = async () => {
      try {
        ...
        const responseTypeStorage = await fetch("http://18.142.0.155:5001/api/TypeStorage/1", {
          method: "GET",
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${token}`
  ...
  useEffect(() => {
    const checkVipStatus = async () => {
      try {
        ...
        const checkVip = await fetch("http://18.142.0.155:5001/api/managerstorage/check-user", {
          method: "GET",
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${token}`
  ...
  const handlePayment = async () => {
    try {
      const response = await fetch('http://18.142.0.155:5001/api/payment', {
        method: 'POST',
        ...
        body: JSON.stringify({
          ...
          OrderType: 'other',
          returnUrl:`http://18.142.0.155:5001/api/payment/confirm?userId=${user?.id}&typeStorageId=${typeStorage?.id}`,
        }),
      });
